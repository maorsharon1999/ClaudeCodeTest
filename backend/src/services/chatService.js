...
async function getThreadsForUser(userId) {
  ...
  const threadsResult = await pool.query(
    `SELECT
       ...
       lm.voice_note_url AS last_msg_voice_note_url,
     ...
     LEFT JOIN LATERAL (
       SELECT id, body, voice_note_url, sent_at, sender_id
       FROM chat_messages
  ...
  return threadsResult.rows.map(row => ({
    ...
    last_message: row.last_msg_id ? {
      body: row.last_msg_body !== null
        ? row.last_msg_body
        : (row.last_msg_voice_note_url ? 'Voice note' : null),
...
async function getMessages(threadId, callerId, { before = null, limit = 50 } = {}) {
  ...
  const msgResult = await pool.query(
    `SELECT id, body, voice_note_url, voice_note_duration_s, sent_at, sender_id
     FROM chat_messages
     WHERE thread_id = $1
  ...
  return {
    messages: rows.map(row => ({
      ...
      voice_note_duration_s: row.voice_note_duration_s,
      sent_at: row.sent_at,
...
async function addVoiceNote(userId, threadId, voiceNoteUrl, durationS) {
  if (!UUID_RE.test(threadId)) {
    throw makeError(404, 'NOT_FOUND', 'Thread not found.');
  }
  const dur = parseInt(durationS, 10);
  if (isNaN(dur) || dur < 1 || dur > 60) {
    throw makeError(400, 'VALIDATION_ERROR', 'duration_s must be between 1 and 60.');
  }
  ...
  const insertResult = await pool.query(
    `INSERT INTO chat_messages (thread_id, sender_id, voice_note_url, voice_note_duration_s)
     VALUES ($1, $2, $3, $4)
     RETURNING id, voice_note_url, voice_note_duration_s, sent_at`,
    [threadId, userId, voiceNoteUrl, dur]
  );
...
module.exports = { getThreadsForUser, getMessages, sendMessage, addVoiceNote };
